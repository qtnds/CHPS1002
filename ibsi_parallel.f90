! =============================================================================
! ibsi_parallel.f90 - Version OpenMP parallélisée du code IBSI
! =============================================================================
! Stratégie de parallélisation :
!   - La boucle externe sur les paires (a,b) est parallélisée via OMP PARALLEL DO
!   - La réduction de dg_sum est gérée localement par thread (tableaux)
!   - L'affichage des performances compare séquentiel vs. parallèle
! =============================================================================
! Compilation :
!   gfortran -O2 -fopenmp -o ibsi_parallel ibsi_parallel.f90
!   export OMP_NUM_THREADS=N   (ou ajuster via --nthreads)
! =============================================================================
...
CONTAINS
  ...
  ! -----------------------------------------------------------------------
  ! Densité électronique proatomique de Slater (IAS)
  ! -----------------------------------------------------------------------
  ...
  ! -----------------------------------------------------------------------
  ! Gradient de la densité proatomique
  ! -----------------------------------------------------------------------
  ...
  ! -----------------------------------------------------------------------
  ! Intégrande de l'IBSI pour une paire (A,B)
  ! -----------------------------------------------------------------------
  ...
  ! -----------------------------------------------------------------------
  ! NOYAU PARALLÈLE : calcul de toute la matrice IBSI via OpenMP
  !
  ! Stratégie :
  !   - Boucle externe sur les paires (a,b) : OMP PARALLEL DO avec SCHEDULE(DYNAMIC)
  !     car le coût peut varier selon la distance inter-atomique.
  !   - La triple boucle i/j/k (grille 3D) est séquentielle à l'intérieur
  !     de chaque thread → pas de faux partage, chaque thread accumule
  !     son propre dg_sum local.
  !   - ibsi_matrix est une variable SHARED mais chaque (a,b) est écrit
  !     par un seul thread → pas de race condition.
  ! -----------------------------------------------------------------------
  SUBROUTINE compute_ibsi_matrix_omp(natoms, xyz, ZAatoms, &
                                      xmin, ymin, zmin, nx, ny, nz, dx, dV, &
                                      ibsi_matrix)
    USE OMP_LIB
    INTEGER, INTENT(in) :: natoms, nx, ny, nz
    REAL(8), INTENT(in) :: xyz(3,natoms), xmin, ymin, zmin, dx, dV
    INTEGER, INTENT(in) :: ZAatoms(natoms)
    REAL(8), INTENT(out) :: ibsi_matrix(natoms, natoms)

    INTEGER :: a, b, i, j, k, npairs, ipair
    INTEGER :: aa_list(natoms*(natoms-1)/2), bb_list(natoms*(natoms-1)/2)
    REAL(8) :: x, y, z, dg_sum, d_ab
    REAL(8) :: t_start, t_end

    ! --- Construire la liste linéaire des paires pour un bon équilibrage ---
    npairs = 0
    DO a = 1, natoms-1
      DO b = a+1, natoms
        npairs = npairs + 1
    ...
    ! ====================================================================
    ! BOUCLE PARALLÈLE PRINCIPALE
    ! Chaque thread traite un sous-ensemble de paires (a,b).
    ! dg_sum est une variable PRIVATE → chaque thread possède sa copie.
    ! SCHEDULE(DYNAMIC,1) : distribution dynamique pour équilibrer la charge
    ! ====================================================================
    !$OMP PARALLEL DO DEFAULT(NONE) &
    !$OMP   SHARED(natoms, xyz, ZAatoms, ibsi_matrix, &
    !$OMP          xmin, ymin, zmin, nx, ny, nz, dx, dV, &
    !$OMP          npairs, aa_list, bb_list) &
    !$OMP   PRIVATE(ipair, a, b, i, j, k, x, y, z, dg_sum, d_ab) &
    ...
    DO ipair = 1, npairs
      ...
      ibsi_matrix(a,b) = (dg_sum * dV) / (d_ab**2)
      ibsi_matrix(b,a) = ibsi_matrix(a,b)
    END DO
    !$OMP END PARALLEL DO

    t_end = OMP_GET_WTIME()
  END SUBROUTINE compute_ibsi_matrix_omp
...
200 PRINT *, "ERREUR: Impossible d'ouvrir le fichier: ", TRIM(filename); STOP
  ...
  SUBROUTINE read_xyz_fragments(filename, has_fragments, frag1_start, frag1_end, frag2_start, frag2_end)
    ...
    sel_a = line(pos_sel_a+12:pos_sel_b-1); sel_a = ADJUSTL(sel_a)
    sel_b = line(pos_sel_b+12:);            sel_b = ADJUSTL(sel_b)
...
PROGRAM ibsi_analysis
  ...
  INTEGER :: nfrags, nthreads
  INTEGER, ALLOCATABLE :: fragment_id(:)
  LOGICAL :: has_fragments

  ! --- Chronomètres ---
  REAL(8) :: t_total_start, t_total_end
  REAL(8) :: t_grid_start, t_grid_end
  REAL(8) :: t_seq_ref           ! temps séquentiel de référence (fourni ou estimé)
  REAL(8) :: speedup, efficiency
  LOGICAL :: seq_time_provided
  CHARACTER(len=64) :: seq_time_str
  ...
  threshold_nc   = 0.02d0
  nthreads       = -1            ! -1 = utilise OMP_NUM_THREADS
  t_seq_ref      = -1.0d0
  seq_time_provided = .FALSE.
    ...
    SELECT CASE(TRIM(arg))
      ...
      CASE("--seuil1")
        ...
        IF (iostat_val /= 0) THEN
          PRINT *, "ERREUR: Valeur invalide pour --seuil1:", TRIM(temp_val); STOP
        END IF
      CASE("--seuil2")
        iarg = iarg + 1
        CALL GET_COMMAND_ARGUMENT(iarg, temp_val)
        ...
        IF (iostat_val /= 0) THEN
          PRINT *, "ERREUR: Valeur invalide pour --seuil2:", TRIM(temp_val); STOP
        END IF
      CASE("--nthreads")
        ! Permet de forcer le nombre de threads sans modifier OMP_NUM_THREADS
        iarg = iarg + 1
        CALL GET_COMMAND_ARGUMENT(iarg, temp_val)
        READ(temp_val, *, IOSTAT=iostat_val) nthreads
        IF (iostat_val /= 0) THEN
          PRINT *, "ERREUR: Valeur invalide pour --nthreads:", TRIM(temp_val); STOP
        END IF
        CALL OMP_SET_NUM_THREADS(nthreads)
      CASE("--tseq")
        ! Temps séquentiel de référence (en secondes) pour calculer le speedup
        iarg = iarg + 1
        CALL GET_COMMAND_ARGUMENT(iarg, seq_time_str)
        READ(seq_time_str, *, IOSTAT=iostat_val) t_seq_ref
        IF (iostat_val /= 0) THEN
          PRINT *, "ERREUR: Valeur invalide pour --tseq:", TRIM(seq_time_str); STOP
        END IF
        seq_time_provided = .TRUE.
    END SELECT
    iarg = iarg + 1
  END DO

  IF (TRIM(input_dir) == "") THEN
    PRINT *, "Usage: ./ibsi_parallel --input <dossier> [options]"
    PRINT *, ""
    PRINT *, "Options:"
    ...
    PRINT *, "  --nthreads  : Nombre de threads OpenMP              (defaut: OMP_NUM_THREADS)"
    PRINT *, "  --tseq      : Temps séquentiel ref. (s) pour speedup (optionnel)"
    PRINT *, ""
    PRINT *, "Exemple:"
    PRINT *, "  ./ibsi_parallel --input mol_dir --seuil1 0.05 --nthreads 8 --tseq 120.5"
    STOP
  END IF
  ...
  ! -------------------------------------------------------------------------
  PRINT *, "=========================================================="
  PRINT *, "  ANALYSE IBSI - VERSION OPENMP PARALLELE"
  PRINT *, "=========================================================="
  ...
  ! =========================================================================
  PRINT *, "=========================================================="
  PRINT *, "  CONFIGURATION OpenMP"
  PRINT *, "=========================================================="
  !$OMP PARALLEL
  !$OMP MASTER
  nthreads = OMP_GET_NUM_THREADS()
  PRINT '(A,I4)', "  Nombre de threads actifs    : ", nthreads
  PRINT '(A,I4)', "  Processeurs logiques disponibles : ", OMP_GET_NUM_PROCS()
  PRINT '(A,I4)', "  Threads max configurés       : ", OMP_GET_MAX_THREADS()
  !$OMP END MASTER
  !$OMP END PARALLEL
  PRINT *, "  Stratégie : OMP PARALLEL DO sur les paires (a,b)"
  PRINT *, "              SCHEDULE(DYNAMIC,1) - équilibrage dynamique"
  PRINT *, "  Granularité : une paire (a,b) par tâche"
  PRINT *, "  Pas de race condition : chaque paire est traitée"
  PRINT *, "                          par un seul thread"
  PRINT *, ""
  ...
  PRINT *, "=========================================================="
  PRINT *, "  GRILLE D'INTEGRATION"
  PRINT *, "=========================================================="
  WRITE(*,'(A,I6,A,I6,A,I6,A,I14,A)') &
    "  Dimensions : ", nx, " x ", ny, " x ", nz, &
    "  =", nx*ny*nz, " points"
  WRITE(*,'(A,F8.4,A)') "  Pas de grille (dx) : ", dx, " bohr"
  WRITE(*,'(A,ES12.4,A)') "  Volume élémentaire : ", dV, " bohr^3"
  WRITE(*,'(A,I6)')       "  Paires d'atomes    : ", natoms*(natoms-1)/2
  WRITE(*,'(A,ES12.4,A)') "  Opérations flot.   : ~", &
    REAL(natoms*(natoms-1)/2, 8) * REAL(nx*ny*nz, 8) * 20.0d0, " FLOPs"
  PRINT *, ""
  ...
  ! =========================================================================
  ! PASSE 1 : CALCUL PARALLÈLE DE LA MATRICE IBSI
  ! =========================================================================
  PRINT *, "=========================================================="
  PRINT *, "  PASSE 1 : CALCUL PARALLELE DE LA MATRICE IBSI"
  PRINT *, "=========================================================="
  PRINT *, ""
  PRINT *, "  [Calcul en cours...]"

  t_grid_start = OMP_GET_WTIME()

  CALL compute_ibsi_matrix_omp(natoms, xyz, ZAatoms, &
                                xmin, ymin, zmin, nx, ny, nz, dx, dV, &
                                ibsi_matrix)

  t_grid_end = OMP_GET_WTIME()
  ...
  PRINT *, "=========================================================="
  WRITE(*,'(A,F12.3,A)') "  Temps calcul IBSI (parallele) : ", &
    t_grid_end - t_grid_start, " secondes"
  WRITE(*,'(A,I4,A)')     "  Threads utilises              : ", nthreads, " threads"
  WRITE(*,'(A,F12.3,A)') "  Temps moyen par paire         : ", &
    (t_grid_end - t_grid_start) / MAX(1, natoms*(natoms-1)/2), " s/paire"

  IF (seq_time_provided .AND. t_seq_ref > 0.0d0) THEN
    speedup    = t_seq_ref / (t_grid_end - t_grid_start)
    efficiency = speedup / REAL(nthreads, 8) * 100.0d0
    PRINT *, ""
    WRITE(*,'(A,F12.3,A)') "  Temps séquentiel (référence)  : ", t_seq_ref, " secondes"
    WRITE(*,'(A,F10.2,A)') "  Speedup mesuré                : ", speedup, "x"
    WRITE(*,'(A,F8.1,A)')  "  Efficacité parallele          : ", efficiency, " %"
    WRITE(*,'(A,F10.2,A)') "  Temps idéal (speedup linéaire): ", &
      t_seq_ref / REAL(nthreads, 8), " s"
    IF (efficiency >= 80.0d0) THEN
      PRINT *, "  -> Parallelisation EXCELLENTE (>= 80 %)"
    ELSE IF (efficiency >= 60.0d0) THEN
      PRINT *, "  -> Parallelisation BONNE (>= 60 %)"
    ELSE
      PRINT *, "  -> Parallelisation FAIBLE (<  60 %) - overhead possible"
    END IF
  ELSE
    PRINT *, "  (Fournir --tseq <secondes> pour afficher le speedup)"
  END IF
  PRINT *, "=========================================================="
  PRINT *, ""

  ! =========================================================================
  ! PASSE 1 : Classification des liaisons
  ! =========================================================================
  ...
  WRITE(unit_res,*) "# Resultats IBSI (version OpenMP)"
  WRITE(unit_res,*) "# Dossier:", TRIM(input_dir)
  WRITE(unit_res,*) "# Nombre d'atomes:", natoms
  WRITE(unit_res,*) "# Threads:", nthreads
  ...
  PRINT *, "  BILAN FINAL"
  PRINT *, "=========================================================="
  WRITE(*,'(A,F12.3,A)') "  Temps total (mur)  : ", t_total_end - t_total_start, " secondes"
  WRITE(*,'(A,F12.3,A)') "  dont calcul IBSI   : ", t_grid_end - t_grid_start,  " secondes"
  WRITE(*,'(A,F6.1,A)')  "  Fraction parallelee: ", &
    (t_grid_end - t_grid_start) / (t_total_end - t_total_start) * 100.0d0, " % du temps total"
  IF (seq_time_provided .AND. t_seq_ref > 0.0d0) THEN
    speedup    = t_seq_ref / (t_grid_end - t_grid_start)
    efficiency = speedup / REAL(nthreads, 8) * 100.0d0
    WRITE(*,'(A,F10.2,A)') "  Speedup final      : ", speedup, "x"
    WRITE(*,'(A,F8.1,A)')  "  Efficacite         : ", efficiency, " %"
  END IF
  PRINT *, "=========================================================="
  PRINT *, ""
